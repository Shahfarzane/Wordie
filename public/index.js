...
notbtn.onclick = () => {
  donebtn.className = "ui button";
  notbtn.className = "ui active button";
  container.innerHTML = "";
  STATE.forEach((e) => {
    if (!e.learned) addCard(e);
  });
};

donebtn.onclick = () => {
  notbtn.className = "ui button";
  donebtn.className = "ui active button";
  container.innerHTML = "";
  STATE.forEach((e) => {
    if (e.learned) addCard(e);
  });
...
function addCard(input) {
  ...
  card.value = input.id;
  card.onclick = (e) => {
    if (e.target.className === "ui master checkbox") return;
...
function addImgCard(input) {
  ...
  div.setAttribute("class", "four column row");
